...
const TableComponent = () => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      require("datatables.net-bs5"); // Load DataTables Bootstrap 5
        ...
        <div className="card">
          ...
          <div className="card-body">
            <div className="table-responsive">
              <table
                id="basic-datatables"
                className="table table-bordered-bottom test-table"
              >
                <thead>
                  <tr>
                    <th>ID </th>
                  ...
                  {tableData.length > 0 ? (
                    tableData.map((row: any, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          <span
                            className={`text-center ps-1 pe-1 p-1 rounded-2 ${getStatusClass(
                              row.status
                            )}`}
                          >
                            {row.status}
